...
server.listen(expressPort, () => console.info(`Rest API para el dashboard se encuentra escuchando el puerto ${expressPort}`))

const ioAdapterRedis = ioSocket.conectionSocketIORedis(server)

const iosocket = ioAdapterRedis.sockets.on('connection', (socket) => {
	console.info(`Connection to client established ${socket.id}`)
	...
	socket.on('broadcastAgentes', () => {
		console.info('Enviando mensaje desde el frontPanel de supervision, para enviar mensajes masivos')
	})

	socket.on('disconnect', () => {
		console.info(`Se ha eliminado el socketID: ${socket.id}`)
 ...
 */
const amiAsterisk = new AmiAsterisk()
amiAsterisk.conectionAsterisk()
