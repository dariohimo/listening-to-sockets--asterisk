class SearchDataEmitAsterisk {
	...
	getNumberExtension (data) {
		...
		if (event === 'NewConnectedLine') return this.eventNewConnectedLine(data)
		if (event === 'AgentConnect') return this.eventAgentConnect(data)
	...
	eventNewConnectedLine (data) {
		const uniqueID = data['Uniqueid']
		const linkedid = data['Linkedid']
		return (uniqueID === linkedid) ? data['ConnectedLineNum'] : data['CallerIDNum']
	}

	eventAgentConnect (data) {
		return data['DestCallerIDNum']
	}
