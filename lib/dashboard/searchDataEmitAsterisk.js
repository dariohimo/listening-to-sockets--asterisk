...
class SearchDataEmitAsterisk {
  ...
  eventNewState (data) {
    return (data['ConnectedLineNum'].length === 4) ? this.data['ConnectedLineNum'] : data['CallerIDNum']
  }

  eventHangup (data) {
    let numberExtension = false
    ...
    if (context[0] === 'nivel') {
      if (data['ChannelStateDesc'] === 'Up') {
        ...
        } else return (uniqueID === linkedid) ? this.extractNumberExtension(data) : callerIDNum
