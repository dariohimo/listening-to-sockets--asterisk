...
const eventsOthers = new EventsOthers()
			...
			.then(() => {
				client
					...
					.on('event', event => {
						 ...
						 */
						if (event.Event === 'QueueMemberPause') { eventsOthers.queueMemberPause(event)	}

						/**
						 * [Capturar el evento Queue Member Added que se produce en el servicio de asterisk]
						 */
						if (event.Event === 'QueueMemberAdded') { eventsOthers.queueMemberPause(event)	}
