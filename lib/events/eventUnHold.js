	...
	unHold(sendData) {
		...
		if(len(dataPreUpdate) > 0){
			...
			**/
			...
			// Puede validarse si el uniqueid y linkedid son iguales
			...
			if (len(dataPreUpdate.second_outbound_phone) > 0) secondCall = (dataPreUpdate.second_outbound_phone === dataEmitAsterisk['Exten']) ? this.validateUnHoldinSecondCall(sendData, true) : this.validateUnHoldinSecondCall(sendData, false)
