...
class EventHangUp {
	hangUp(sendData) {
		...
		if (len(dataPreUpdate) > 0) {
			...
			**/
			...
			statusAsteriskJSON = (dataPreUpdate.second_status_call === 1) ? this.asteriskResponse(sendData, true) : this.asteriskResponse(sendData, false) 
	...
	endsTheOutboundCall (sendData) {
		const dataEmitAsterisk = sendData.dataEmitAsterisk
		const dataPreUpdate = sendData.dataPreUpdate
		const eventName = (dataPreUpdate.event_id === 13) ? 'Ring Outbound' : helper.nameEvent(dataPreUpdate.event_id)
		return {
