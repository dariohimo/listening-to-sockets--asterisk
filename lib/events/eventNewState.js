				...
				**/
				if (exten.length > 4 && uniqueID === linkedID) {
					...
					if(preEventID === '16' || preEventID === '17') {
						...
						actionJSON = (uniqueID === linkedID) ? newStateRing.internalEmisor(sendData) : newStateRing.internalReceptor(sendData)
					}
				}
			}
		}
		...
		**/
		if (channelStateDesc === 'Up' && len(dataPreUpdate) > 0) {
			if (connectedLineNum !== '<unknown>') {
				...
				if (context[0] === 'context') {
					...
					if(uniqueID === linkedID){
						...
						if(dataPreUpdate.event_id === '16' || dataPreUpdate.event_id === '17'){
							...
							// console.log(dataPreUpdate.event_id)
							// answerInternal = (dataPreUpdate.event_id === '27') ? newStateAnswer.receptorBlindTransferCall(sendData) : newStateAnswer.emisorConnectInternalCall(sendData)
				...
				**/
				/*
				answerExternOutgoing = (uniqueID === linkedID) ? this.ExternOutgoingCall(sendData, false) : this.ExternOutgoingCall(sendData, true)
				actionJSON = Object.assign(this.validateInboundCall(sendData, false),answerExternOutgoing)
				*/
			}
		}else if(context[0] === 'nivel' && uniqueID != linkedID && channelStateDesc === 'Up'){
			//answerExternOutgoing = this.ExternOutgoingCall(sendData, false, true)
			//actionJSON = Object.assign(this.validateInboundCall(sendData, true, true),answerExternOutgoing)
		}
