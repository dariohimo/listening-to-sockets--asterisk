...
const request = new Request({
  ...
  apiName: process.env.apiDetailDashboard
})

class DetailDashboard {
  getRequest (data, event, urlController, msjLog, isSearchAnnexed = false) {
    console.log(event)
    return new Promise(async (resolve, reject) => {
      let dataPreUpdate = {}
      if (isSearchAnnexed) dataPreUpdate = await this.getRequestSearchAnnexed(event, data)
      let agent = await getStructure(event, data, dataPreUpdate)
      if (agent) {
  ...
  getRequestSearchAnnexed (event, data) {
    return new Promise(async (resolve, reject) => {
      ...
      if (annexed) resolve(await request.send('/search', 'POST', {'agent_annexed': annexed}))
      else reject(new Error('Error al buscar la dataPreUpdate debido a que no se encuentra con un anexo'))
    })
  }

  shows () {
    return new Promise((resolve, reject) => {
      request.send('/', 'GET', null).then(dataAgent => { return resolve(dataAgent) }).catch(err => { return reject(err) })
    })
  }

  search (data) { return this.getRequest(data, 'searchAgent', '/search', 'Search segun el anexo el EventId del Agente') }

  memberPause (data) { return this.getRequest(data, 'memberPause', '/updatePause', 'Pausing Agent') }
  memberAdd (data) { return this.getRequest(data, 'memberAdd', '/searchAndUpdate', 'Dashboard : Adding Agent') }

  ringInbound (data) { return this.getRequest(data, 'newConnectedLine', '/searchAndUpdate', 'Dashboard : Ring Inbound') }
  answerInbound (data) { return this.getRequest(data, 'agentConnect', '/searchAndUpdate', 'Dashboard : Answer Inbound') }
  ringAnswerOutbound (data) { return this.getRequest(data, 'newState', '/searchAndUpdate', 'Dashboard : Ring Outbound', true) }
  hangup (data) { return this.getRequest(data, 'hangup', '/searchAndUpdate', 'Dashboard : Hangup', true) }
  hold (data) { return this.getRequest(data, 'hold', '/searchAndUpdate', 'Dashboard : Hold', true) }
  unhold (data) { return this.getRequest(data, 'unHold', '/searchAndUpdate', 'Dashboard : UnHold', true) }

  transferUnattended (data) { return this.getRequest(data, 'blindTransfer', '/transferUnattended', 'Dashboard : Remove Transfer Unattended', true) }
  attendedTransfer (data) { return this.getRequest(data, 'attendedTransfer', '/searchAndUpdate', 'Dashboard : AttendedTransfer') }
}
...
const getAnnexed = (event, data) => {
  ...
  let annexed = ''
  if (datos['CallerIDNum'] !== 'undefined') {
    var context = datos['Context'].split('-')

    if (event === 'hold') annexed = (datos['CallerIDNum'].length > '4') ? extractAnnex(datos) : datos['CallerIDNum']
    if (event === 'unHold') annexed = (datos['CallerIDNum'].length > '4') ? extractAnnex(datos) : datos['CallerIDNum']
    if (event === 'newState') {
      annexed = (datos['ConnectedLineNum'].length === 4) ? datos['ConnectedLineNum'] : datos['CallerIDNum']
    }

    if (event === 'hangup') {
      if (datos['ConnectedLineNum'] !== '<unknown>' && context[0] === 'context') annexed = datos['ConnectedLineNum']
      if (datos['ConnectedLineNum'] === '<unknown>' && context[0] === 'feature') annexed = datos['CallerIDNum']

      if (context[0] === 'nivel') {
        if (datos['ChannelStateDesc'] === 'Up') {
          if (datos['ConnectedLineNum'] !== '<unknown>') {
            if (datos['Exten'].length === 0) annexed = datos['CallerIDNum']
            else annexed = (datos['Exten'].length <= 4) ? datos['CallerIDNum'] : datos['ConnectedLineNum']
          } else annexed = (datos['Uniqueid'] === datos['Linkedid']) ? extractAnnex(datos) : datos['CallerIDNum']
        } else annexed = (datos['ChannelStateDesc'] === 'Ring' && datos['Uniqueid'] === datos['Linkedid']) ? extractAnnex(datos) : datos['CallerIDNum']
      }
    }
  }

  if (datos['TransfererCallerIDNum'] !== 'undefined') {
    if (event === 'blindTransfer') annexed = datos['TransfererCallerIDNum']
  }
