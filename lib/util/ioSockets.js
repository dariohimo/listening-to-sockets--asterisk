...
class ioSockets {
	...
	createRoomFrontPanel (socket, data) {
		...
		console.info(`Creando Sala frontPanel -> ${nameRoom}`)
	}
	...
	leaveRoomFrontPanel (socket, data) {
		...
		console.info(`Eliminando Sala frontPanel -> ${nameRoom}`)
	}
	...
	createRoomDashboard (socket, data) {
		...
		socket.join(nameRoom)
		console.info(`Creando Sala Dashboard -> ${nameRoom}`)
	}
	...
	sendEmitDashboard(nameEmit, data ) {
		if(len(data) != 0) {
			let nameProyect = (data.nameProyect) ? data.nameProyect : data.name_proyect
			console.info(`Emitiendo Socket -> dashboard:${nameProyect} -> Evento:${nameEmit}`)
	...
	sendEmitFrontPanel(data){
		if(len(data) != 0) {
			let nameProyect = (data.nameProyect) ? data.nameProyect : data.name_proyect
			console.info(`Emitiendo Socket -> frontPanel:${nameProyect}:${data.agent_user_id}`)
