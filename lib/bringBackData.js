...
class BringBackData {
   ...
   */
	handlerError (msj, err) {
		console.error(`handlerError ---> ${msj} : ${err}`)
	}
	...
	updateUserToDashboard (data) {
		...
		const valuesPanelOthers = ['1', '2', '3', '4', '5', '6', '7','11']
	...
	listCallsWaiting (data) {
		detailCallsWaiting.show(data).then(data => {
			data.forEach(item => ioSocket.sendEmitDashboard('AddCallWaiting',item))
		}).catch(err => this.handlerError('Error al obtener las llamadas encoladas', err))
	}
