...
function getStructure (event, data) {
  ...
  if (event === 'QueueCallerJoin') {
    callWaiting = {
      ...
      'name_number': datos.CallerIDName,
      'name_queue': datos.Queue,
      'start_call': (new Date()).getTime(),
      'number_annexed': (datos.ConnectedLineNum === '<unknown>') ? '' : datos.ConnectedLineNum,
      'name_agent': (datos.ConnectedLineName === '<unknown>') ? '' : datos.ConnectedLineName
    }
    return callWaiting
  }
}
